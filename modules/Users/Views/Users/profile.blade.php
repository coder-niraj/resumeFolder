...
@section('content')
    <div class="d-flex">
        <div class="">
            @include('user::components.sidebar', ['active' => 'profile'])
        </div>
                ...
                <div class="container"
                    ...
                    <div class="profile">
                        <div class="avatar py-5 px-3 d-flex align-items-center gap-3">
                            @if ($userObj->avatar)
                                <img src="{{ asset('storage/' . $userObj->avatar) }}" class="rounded-circle" width="120"
                            ...
                            <div class="name" style="line-height: 35px;">
                                <div class="fs-3">{{ $userObj->firstname . ' ' . $userObj->lastname }}</div>
                                <div>{{ $userObj->email }}</div>

                            </div>
                        </div>
                        <div class=" profile-details">
                            <form class="row p-3 g-3 needs-validation" novalidate action="{{ route('user.update') }}"
                                method="POST" enctype="multipart/form-data">
                                @csrf
                                <div class="col-6">
                                    <label for="validationCustom01" class="form-label">First name</label>
                                    <input type="text" class="form-control input-bg " j value="{{ $userObj->firstname }}"
                                ...
                                <div class="col-6">
                                    ...
                                    <input type="text" class="form-control input-bg " j value="{{ $userObj->lastname }}"
                                ...
                                <div class="col-6">
                                    ...
                                    <input type="number" class="form-control input-bg " value="{{ $userObj->phone }}"
                                        name="phone">
                                    @error('phone')
                                        <div class="text-danger">{{ $message }}</div>
                                    @enderror
                                ...
                                <div class="row py-5 px-1">
                                    ...
                                    <div class="col-3">
                                        <a href="{{ route('user.password') }}" class="btn btn-dark w-100">
                                            change password
                                        </a>
                                    </div>
                                </div>
